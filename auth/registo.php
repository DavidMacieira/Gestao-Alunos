...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // 1. Inserir na tabela 'utilizadores' (perfil_id 1 = Aluno conforme o SQL)
        $sqlUser = "INSERT INTO utilizadores (nome, email, password, perfil_id) VALUES (?, ?, ?, 1)";
        $stmtUser = $pdo->prepare($sqlUser);
        $stmtUser->execute([$nome, $email, $password]);

        // 2. Inserir na tabela 'alunos' (ajustado às colunas do seu SQL)
        $sqlAluno = "INSERT INTO alunos (nome, email, password, curso_id) VALUES (?, ?, ?, ?)";
        $stmtAluno = $pdo->prepare($sqlAluno);
        $stmtAluno->execute([$nome, $email, $password, $curso_id]);
